...
- (NSArray *)imageArray
{
    if (!_imageArray) {
        ...
        NSArray *arr = @[@"1",@"2",@"3",@"4",@"5",@"6.jpeg",@"7.jpeg",@"8.jpeg",@"9.jpeg",@"10.jpeg",@"11.jpeg",@"12.jpeg",@"13.jpeg",@"14.jpeg"];
...
- (YBPasterScrollView *)pasterScrollView
{
    if (!_pasterScrollView) {
        ...
        _pasterScrollView.backgroundColor = [UIColor whiteColor];
...
- (void)setupUI
{
    ...
    UIImageView *pasterImageView = [[UIImageView alloc]initWithFrame:CGRectMake(20, 0, FULL_SCREEN_W - 40, FULL_SCREEN_H - 200)];
    pasterImageView.image = self.originalImage;
	pasterImageView.image = [pasterImageView.image zoomingToScreen];
	
	pasterImageView.frame = CGRectMake(0, 0,  pasterImageView.image.size.width,pasterImageView.image.size.height);
...
- (void)setRightButton
{
    [self initBackButtonWithButtonW:0 buttonH:0 title:@"" titleColor:[UIColor whiteColor] buttonImage:nil negativeSpacer:0 touchBlock:nil];
    [self initRightButtonWithButtonW:50 buttonH:50 title:@"完成" titleColor:[UIColor darkGrayColor] touchBlock:^(UIButton *rightButton) {
    }];
